...
[red]*[Since v.1.1.0-XC]* [blue]*Added support for the Handover Protocol that allows passing the control between many apps (page inbox included).* Handover protocol allows you to have a bot as a primary receiver and a customer care agent (via page inbox) as a secondary receiver, among other setups.
...
== Reference
=== Instantiation
==== with default Latest API Version, default User Profile Fields, default HTTP-Client (okHttp)
[source,java,indent=0]
----
        final Messenger messenger = Messenger.create("PAGE_ACCESS_TOKEN", "APP_SECRET", "VERIFY_TOKEN");
----

==== with custom API Version and default User Profile Fields, default HTTP-Client (okHttp)
[source,java,indent=0]
----
        final String version = "v2.6";
        final Messenger messenger = Messenger.create("PAGE_ACCESS_TOKEN", "APP_SECRET",
                "VERIFY_TOKEN", Optional.of(version));
----

==== with custom API Version (validating its correctness at date of this release) and default User Profile Fields, default HTTP-Client (okHttp)
[source,java,indent=0]
----
        final String version = Messenger.Version.getVersionFromString(version).getVersionUrl();
        final Messenger messenger = Messenger.create("PAGE_ACCESS_TOKEN", "APP_SECRET",
                "VERIFY_TOKEN", Optional.of(version));
----

==== with custom User Profile Fields and default API Version and HTTP-Client (okHttp)
[source,java,indent=0]
----
        final String userProfileFields="first_name,last_name"
        final Messenger messenger = Messenger.create("PAGE_ACCESS_TOKEN", "APP_SECRET",
                "VERIFY_TOKEN", Optional.empty(), Optional.of(userProfileFields));
----

==== with custom HTTP-Client and default API Version and User Profile Fields
[source,java,indent=0]
----
...
==== query user information by user ID with Custom User Profile Fields
[source,java,indent=0]
----
        final String userProfileFields = "first_name,last_name"
        final UserProfile userProfile = messenger.queryUserProfile(userId, userProfileFields);
----
