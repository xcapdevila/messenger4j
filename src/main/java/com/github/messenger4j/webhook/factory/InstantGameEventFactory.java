...
public class InstantGameEventFactory implements BaseEventFactory<InstantGameEvent> {
	...
	@Override
	public InstantGameEvent createEventFromJson(JsonObject messagingEvent, BaseEventType baseEventType) {
		...
		final String gameId = getPropertyAsString(messagingEvent, PROP_GAME_PLAY, PROP_GAME_ID).orElseThrow(IllegalArgumentException::new);
		final String playerId = getPropertyAsString(messagingEvent, PROP_GAME_PLAY, PROP_PLAYER_ID).orElseThrow(IllegalArgumentException::new);
		final String contextType = getPropertyAsString(messagingEvent, PROP_GAME_PLAY, PROP_CONTEXT_TYPE).orElseThrow(IllegalArgumentException::new);
		...
		return new InstantGameEvent(senderId, recipientId, timestamp, baseEventType, gameId, playerId, contextType, contextId, score, payload);
	}
}
